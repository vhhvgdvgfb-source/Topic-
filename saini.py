...
def get_mps_and_keys(api_url):
    """
    Get MPD URL and decryption keys from the Classplus API
    Supports both DRM and non-DRM videos
    
    Args:
        api_url: The API endpoint URL with parameters
        
    Returns:
        tuple: (mpd_url, keys) for DRM videos
        str: signed_url for non-DRM videos
        None: if failed
    """
    try:
        ...
        # Check if it's a DRM video (has MPD/mpd_url and KEYS/keys)
        ...
        if mpd and keys:
            if not mpd or not keys:
                print(f"❌ Invalid DRM response: missing MPD or KEYS")
                return None, None
                
            print(f"✅ Successfully got MPD and {len(keys)} keys")
            print(f"   MPD: {mpd[:100]}...")
            print(f"   Keys: {keys}")
            return mpd, keys
        
        # Check if it's a non-DRM video (has signed url)
        elif 'url' in response_json:
            signed_url = response_json.get('url')
            
            if not signed_url:
                print(f"❌ Invalid non-DRM response: missing url")
                return None
                
            print(f"✅ Successfully got signed URL")
            return signed_url
